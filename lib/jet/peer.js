...
Peer.prototype.fetchFake = function (fetchParams, f, callbacks) {
	...
	fetchParams.id = id;
	var ref = {};
	var fetchDispatcher = createFetchDispatcher(fetchParams, f, ref);
	...
	if (this.elements === undefined) {
		...
		that.jsonrpc.service('fetch', {}, undefined, {
			success: function (fetchSimpleId) {
				...
				fetchCommon.fetchCore(that, that.elements, fetchParams, wrappedFetchDispatcher, callbacks && callbacks.success);
			}
		});
	} else {
		fetchCommon.fetchCore(that, that.elements, fetchParams, wrappedFetchDispatcher, callbacks && callbacks.success);
	...
	ref.unfetch = function (callbacks) {
		fetchCommon.unfetchCore(that, that.elements, fetchParams);
	...
	ref.isFetching = function () {
		return that._fetchers[that.id + fetchParams.id] && true || false;
	};
