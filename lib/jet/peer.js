  ...
  var Peer = function (config) {
    ...
    msock.sendMessage = function(message) {
    ...
    var add = function (desc, dispatch, callback) {
      ...
      params.value = desc.value; // only for states
      params.timeout = desc.timeout; // optional
    ...
    self.set = function (path, value, callback) {
      var params = {
        ...
        value: value,
        timeout: callback && callback.timeout,
        valueAsResult: callback && callback.valueAsResult
      };
          ...
          dispatch = function (message) {
            ...
            if (isDefined(message.id)) {
              reply = function (result) {
                ...
                if (!isDefined(result.error) && !!!result.dontNotify) {
                    desc.value = newValue;
                    msock.sendMessage({
                      method: 'change',
                      params: {
