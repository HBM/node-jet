 ...
 * disableBatches.
 *
 * Prevents sending multiple queued messages as JSON-RPC batch (Array).
 */
JsonRPC.prototype.disableBatches = function () {
 ...
 * Queue.
 * 
 * Queues a message. Does not actually send the message.
 */
 ...
 * Flush.
 *
 * Flushes/sends all queued messages.
 * If "this.useBatches" is true and more than one message is queued,
 * sends the message as batch (Array).
 */
 ...
 * Service.
 * 
 * Issues a service method call either as JSON-RPC Request (if success and/or error
 * callback are specified) or as JSON-RPC Notification.
 * "method" must be one of the service calls supported by the Daemon, e.g. 
 * "set", "call", "add", "remove" etc.
 * "params" content depends on the "method".
 * "complete" is a optional function which is executed either if no success and/or
 * error function is specified alwys with "true" as argument. 
 * Or - if the service is issued as Request - with "true" as argument in case of success,
 * and with "false" as argument in case of error. 
 */
