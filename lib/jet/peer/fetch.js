...
var Fetcher = function (jsonrpc, params, fetchCb) {
  ...
  var fetchDispatcher = createFetchDispatcher(params, function () {
    if (!resolved) {
      queue.push([this, JSON.parse(JSON.stringify(Array.prototype.slice.call(arguments)))])
      return
    }
    fetchCb.apply(this, arguments)
  }, this)
  ...
  this.fetch = function () {
    resolved = false
    queue = []
    jsonrpc.service('fetch', params, addFetchDispatcher)
      .then(function () {
        resolved = true
        if (queue.length > 0) {
          queue.forEach(function (item) {
            fetchCb.apply(item[0], item[1])
          })
          queue = []
        }
        return true
