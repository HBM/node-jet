...
var FakeFetcher = function (jsonrpc, fetchParams, fetchCb) {
  ...
  if (jsonrpc.fakeContext === undefined) {
    ...
    var fetchSimpleDispatcher = function (message) {
      context.fetchAllPromise.then(function () {
        if (!context.fetching) {
          fetchCommon.fetchCore(context.peer, context.elements, fetchParams, wrappedFetchDispatcher)
    ...
    context.fetchAllPromise = jsonrpc.service('fetch', {}, function (ok, fetchSimpleId) {
  ...
  this.fetch = function () {
    var context = jsonrpc.fakeContext
    if (context.fetching) {
      // defer this but execute before i/o handling (-> setTimeout(, 0) will NOT work.
      setImmediate(function () {
        fetchCommon.fetchCore(context.peer, context.elements, fetchParams, wrappedFetchDispatcher)
      })
    }
    return context.fetchAllPromise
  }
...
var Fetcher = function (jsonrpc, params, fetchCb) {
  ...
  var fetchDispatcher = createFetchDispatcher(params, function () {
    if (!fetchCb) {
      return
    }
    var self = this
  ...
  this.fetch = function () {
    confirmed = jsonrpc.service('fetch', params, addFetchDispatcher)
    return confirmed
  }
}
