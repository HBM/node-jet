...
exports.createDaemon = function() {
    ...
    var createSorter = function(options, notify) {
        ...
        console.log('SORTER', options.sort);
        ...
        var isInRange = function(i) {
            return typeof i === 'number' && (i + 1) >= from && (i + 1) < to;
        };
        ...
        var sorter = function(notification, initializing) {
            ...
            newIndex = index[path] || -1;

            if (isDefined(lastIndex) && newIndex === lastIndex) {
                if (event === 'change') {
                    notify({
                        n: max - from + 1,
                        value: [{
            ...
            if (isIn && wasIn) {
                ...
                stop = max;
                max = max - 1;
            } else if (!isIn && !wasIn && isDefined(lastIndex) && lastIndex < from) {
                start = from;
                stop = max;
            } else {
                return;
    ...
    var fetch = function(peer, message) {
        ...
        if (isDefined(sorter) && !isDefined(sorter.flush) && isDefined(message.id)) {
            peer.sendMessage({
                id: message.id,
                result: true
            });
