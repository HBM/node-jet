...
var NodeMockSock = require('./nodeMockSock').NodeMockSock
...
var createDaemon = function (options) {
  ...
  daemon.listen = function (listenOptions) {
    ...
    if (listenOptions.wsPort || listenOptions.server) {
      ...
      daemon.wsServer.on('connection', function (ws) {
        var peer = peers.add(ws)
        peer.auth = ws.upgradeReq._jetAuth
        var pingMs = listenOptions.hasOwnProperty('wsPingInterval') ? listenOptions.wsPingInterval : 5000
        var pingInterval
        if (pingMs) {
  ...
  daemon.createLocalPeer = function () {
    var nodeMockSock = new NodeMockSock()
    var localPeer = peers.add(nodeMockSock.daemonSock)
    var peer = new Peer({sock: nodeMockSock.peerSock})

    localPeer.on('disconnect', function () {
      daemon.emit('disconnect', localPeer)
    })
    daemon.emit('connection', localPeer)

    return peer
  }

  daemon.close = function () {
    var listenerClosePromise = new Promise(function (resolve) {
      if (daemon.listener) {
        daemon.listener.destroy(function () {
          resolve()
        })
      } else {
        resolve()
    ...
    var wsServerClosePromise = new Promise(function (resolve) {
      if (daemon.wsServer) {
        daemon.wsServer.close(resolve)
      } else {
        resolve()
      }
    })

    return Promise.all([listenerClosePromise, wsServerClosePromise])
  }
  return daemon
}
