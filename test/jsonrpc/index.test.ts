...
describe("Testing JsonRpc",()=>{
    ...
    it("Should test unconnected ",(done)=>{
        ...
        .catch((ex)=> expect(ex.toString()).toEqual("jet.ConnectionClosed: Connection is closed"))
        .then(()=>jsonrpc.publish("_f",{method:"add",path:"foo",value:1}))
        .catch((ex)=> expect(ex.toString()).toEqual("jet.ConnectionClosed: Connection is closed"))
        .then(()=>jsonrpc.close())
        .then(()=>done())
    })
    ...
    it("Should test send",(done)=>{
        ...
        jest.spyOn(sock,"send").mockImplementation((msg)=>{
            expect(msg).toEqual(JSON.stringify({id:"1",method:"add",params:{path:"foo",value:3}}))
            done()
        })
    ...
    it("Should test publish",(done)=>{
        ...
        jest.spyOn(Sock,"Socket").mockImplementation(()=>sock)
        jest.spyOn(sock,"send").mockImplementation((msg)=>{
            expect(msg).toEqual(JSON.stringify({id:"1",method:"_f",params:{method:"add",path:"foo",value:1}}))
            done()
        })
        const jsonrpc= new JsonRPC(new Logger())
        jsonrpc
    ...
    it("Should test batch publish",(done)=>{
        ...
        jest.spyOn(sock,"send").mockImplementation((msg)=>{
            expect(JSON.stringify([
                {id:"1",method:"_f",params:{method:"add",path:"foo",value:1}},
                {id:"2",method:"_f",params:{method:"add",path:"foo",value:2}},
                {id:"3",method:"_f",params:{method:"add",path:"foo",value:3}}
            ])).toContain(msg)
        })
        const jsonrpc= new JsonRPC(new Logger())
        jsonrpc
    ...
    it("Should test invalid request",(done)=>{
        const sock = sockMock()
        jest.spyOn(Sock,"Socket").mockImplementation(()=>sock)
        const logger = new Logger({logname:"Mock",loglevel:LogLevel.error,logCallbacks:[(msg)=>{
            console.log(msg)
            expect(msg).toContain("Mock	error	SyntaxError: Unexpected token I in JSON at position 0")
            done()
    }]})
    ...
    it("Should log socket errors",(done)=>{
        ...
        const logger = new Logger({logname:"Mock",loglevel:LogLevel.error,logCallbacks:[(msg)=>{
            console.log(msg)
            expect(msg).toContain("Mock	error	Error in socket connection:sock error")
            done()
    }]})
        const jsonrpc= new JsonRPC(logger)
        jsonrpc
    ...
    it("Should test incoming request",(done)=>{
        ...
        .then(()=> {
            jsonrpc.addListener("message",(method,msg)=>{
                ...
                expect(msg).toEqual({id:"1",method:"add",params:{method:"add",path:"foo",value:1}})
                done()

            })
            const json = JSON.stringify(
                {id:"1",method:"add",params:{method:"add",path:"foo",value:1}})
            sock.emit("message", {data:json})
        }
        )
        sock.emit("open")
    ...
    it("Should test incoming request array",(done)=>{
        ...
        const messages = [{id:"1",method:"add",params:{method:"add",path:"foo",value:1}},
        {id:"3",method:"add",params:{method:"add",path:"foo",value:1}},
        {id:"4",method:"add",params:{method:"add",path:"foo",value:1}}]
        const jsonrpc= new JsonRPC(new Logger())
    ...
    it("Should test error object response",(done)=>{
        ...
        .then(()=> {
            jsonrpc.send("add",{path:"foo"}).catch((res)=>{
            expect(res.toString()).toEqual("jet.NotFound: No State/Method matching the specified path")
               done()
            })
            const json = JSON.stringify({id:"1",error:{code:INVALID_PARAMS_CODE,data:{pathNotExists:"Foo"}}})
            sock.emit("message", {data:json})
        }
        )
        sock.emit("open")
