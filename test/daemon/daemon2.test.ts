...
describe("Testing Daemon 2 (Notifications)",()=>{
        it("Should test daemon functionality",(done)=>{
            ...
            const daemon = new Daemon({features:{batches:true,asNotification:true,authentication:true,fetch:"full"}})
            daemon.listen({})
            ...
            .then((res)=>{
                expect(res.message).toEqual({"features": {"authentication": true,"asNotification": true, "batches": true, "fetch": "full"}, "name": "node-jet", "protocolVersion": "1.1.0", "version": "2.2.0"})
        ...
        it("Should test get & fetch functionality",(done)=>{
            ...
            //Adding state
            mockServer.simulateMessage(peer,"add",{id:"1",method:"add",params:{path:"bar",value:2}} as MethodRequest)
            .then(()=>mockServer.simulateMessage(peer,"add",AddState("bar2",3)))
            .then(()=>mockServer.simulateMessage(peer,"add",AddState("bar4",5)))
            .then(()=>mockServer.simulateMessage(peer,"change",{id:"4",method:"change",params:{path:"bar",value:1}} as MethodRequest))
            .then(()=>mockServer.simulateMessage(peer,"change",{id:"5",method:"change",params:{path:"bar4",value:6}} as MethodRequest))
            ...
            .then(()=>mockServer.simulateMessage(peer,"get",{id:"7",method:"get",params:{path:{startswith:"bar"}}} as MethodRequest))
            .then((res)=>{
                expect(res.message).toEqual([{"path": "bar", "value": 1}, {"path": "bar2", "value": 3}, {"path": "bar4", "value": 6}])
                expect(res.success).toEqual(true)
            })
            .then(()=>done())
        })
        ...
        it("Should test change functionality",(done)=>{
            ...
            .then(()=>mockServer.simulateMessage(peer,"change",{id:"3",method:"change",params:{path:"bar",value: newValue}} as MethodRequest))
