...
["full", "simple"].forEach(function (fetchType) {
  describe(
    "Fetch (" + fetchType + ") tests with daemon and peer ",
    function () {
      let daemon;
      let peer;
      const port = ++portBase;
            ...
            peer
              ...
              .then(function () {
                a2.remove().then(function () {
                  setTimeout(function () {
                    expect(fetchSpy.callCount).toEqual(3);
                    expect(
                      fetchSpy.calledWith({
                        path: "abc",
                        event: "add",
        ...
        it("startsWith case insensitive", function (done) {
          ...
          states
            ...
            .then(function () {
              ...
              setTimeout(function () {
                expect(fetchSpy.callCount).toEqual(4);
                expect(
                  fetchSpy.calledWith({
                    path: "Aa",
                    event: "add",
        ...
        it("sort by path and differential", function (done) {
          ...
          states.addAll().then(function () {
            ...
            setTimeout(function () {
              ...
              expect(fetchSpy.callCount).toEqual(1);
              expect(fetchSpy.calledWith(expectedChanges, 10)).toBeTruthy();
              expect(fetchOK).toBeTruthy();
              fetcher.unfetch();
              done();
            }, waitTime * 2);
          });
        ...
        it("from / to works", function (done) {
          ...
          states.addAll().then(function () {
            ...
            setTimeout(function () {
              ...
              expect(fetchSpy.callCount).toEqual(1);
              expect(fetchSpy.calledWith(expectedChanges, 3)).toBeTruthy();
              fetcher.unfetch().then(function () {
                done();
              });
            }, waitTime * 2);
        ...
        it("n callback param indicates number of matches within from/to", function (done) {
          ...
          states.addAll().then(function () {
            ...
            setTimeout(function () {
              ...
              expect(fetchSpy.callCount).toEqual(1);
              expect(fetchSpy.calledWith(expectedChanges, 2)).toBeTruthy();
        ...
        it("sort standard (non-differential) feeds callback with complete array", function (done) {
          ...
          states.addAll().then(function () {
            ...
            setTimeout(function () {
              ...
              expect(fetchSpy.callCount).toEqual(1);
              expect(fetchSpy.calledWith(expectedArray)).toBeTruthy();
  ...
  describe("raw message tests", function () {
    ...
    it('"fetch" call returns a string and ignores fetch params and triggers fetch ALL', function (done) {
      ...
      sock.on("message", function (resp) {
        ...
        if (cnt === 0) {
          expect(resp.result).toBeInstanceOf("string");
          fetchId = resp.result;
          expect(resp.id).toEqual(99);
        } else {
          expect(resp.method).toEqual(fetchId);
          expect(resp.params.path).toBeInstanceOf("string");
          expect(resp.params.value).toEqual(123);
        }
        ++cnt;
        if (cnt === 3) {
          done();
    ...
    it("fetching twice fails", function (done) {
      ...
      sock.on("message", function (resp) {
        ...
        if (cnt === 0) {
          expect(resp.result).toBeInstanceOf("string");
          expect(resp.id).toEqual(100);
        } else if (resp.id) {
          expect(resp.error.code).toEqual(-32602);
          expect(resp.error.message).toEqual("Invalid params");
          done();
        }
        ++cnt;
      });
    ...
    it("fetch/unfetch succeeds", function (done) {
      ...
      sock.on("message", function (resp) {
        ...
        if (cnt === 0) {
          expect(resp.result).toBeInstanceOf("string");
          expect(resp.id).toEqual(200);
        } else if (cnt === 3) {
          expect(resp.result).toEqual(true);
          expect(resp.id).toEqual(201);
          done();
        }
        ++cnt;
      });
