...
describe("Testing Peer",()=>{
    ...
    describe ("Should handle daemon messages",()=>{
        ...
        it("Should send different messages",(done)=>{
            ...
            peerMock.addListener = jest.fn().mockImplementation((eventName,callback)=>{
            ...
            .then(()=>{
                ...
                expect(peerMock.respond).toBeCalledWith(undefined, {"path": "foo", "value": 5}, true)
    ...
    describe ("Should connect to daemon",()=>{
        ...
        it("Should fail to connect",(done)=>{
            const connectSpy= jest.fn().mockReturnValue(Promise.reject("could not connect"))
        ...
        it("Should fail to authenticate",(done)=>{
            ...
            const sendSpy= jest.fn().mockImplementation((method,_args)=> (method==="authenticate")?Promise.reject("Wrong credentials"):Promise.resolve({}))
            const jsonrpc = {...fullFetcherPeer(),connect: connectSpy,send: sendSpy}
            jest.spyOn(JsonRPC,"default").mockImplementation(()=>jsonrpc)
        ...
        it("Should connect to peer with credentials",(done)=>{
            ...
            peer.connect().then(()=>{
                expect(connectSpy).toBeCalled()
                expect(sendSpy).toBeCalledWith("info",{})
                expect(sendSpy).toBeCalledWith("authenticate",{"password": "bar","user": "foo"})
                done()
            })
    ...
    describe ("Should test setting a state",()=>{
        it("Should fail to set a state",(done)=>{
            ...
            peer.set("Foo",5,{timeout:5,valueAsResult:true}).catch((ex)=>{
                expect(sendSpy).toBeCalledWith("set",{"path": "Foo","value": 5,"timeout": 5,"valueAsResult": true})
                expect(ex).toBe("invalid path")
                done()
            })
        })
    ...
    describe ("Should test calling a method",()=>{
        it("Should fail to call a method",(done)=>{
            ...
            peer.call("Foo",[5],{timeout:5}).catch((ex)=>{
                expect(sendSpy).toBeCalledWith("call",{"path": "Foo","args": [5],timeout:5})
                expect(ex).toBe("invalid path")
                done()
            })
        })
        it("Should call a method",(done)=>{
            ...
            peer.call("Foo",{"abc":4}).then((res)=>{
                expect(sendSpy).toBeCalledWith("call",{"path": "Foo","args": {"abc":4}})
                expect(res).toEqual({})
                done()
            })
        })
    ...
    describe ("Should test fetching",()=>{
        it("Should fail to fetch",(done)=>{
            ...
            peer.fetch(new Fetcher()).catch((ex)=>{
                expect(sendSpy).toBeCalledWith("fetch",{id: "___f___1",path: {},sort:{}})
                expect(ex).toBe("invalid path")
                done()
            })
        })
        it("Should fetch",(done)=>{
            const sendSpy= jest.fn().mockImplementation((method)=>(method==="info")?Promise.resolve({features:{fetch: "full"}}):Promise.resolve([4,3,2]))
            const jsonrpc = {...fullFetcherPeer(),send: sendSpy}
            jest.spyOn(JsonRPC,"default").mockImplementation(()=>jsonrpc)
            const peer = new Peer()
            peer.connect()
            ...
            .then((res)=>{
                expect(sendSpy).toBeCalledWith("fetch",{id: "___f___1",path: {"startsWith":"a"},sort:{}})
                expect(res).toEqual([4,3,2])
            })
            .then(()=>peer.fetch(new Fetcher().path("equals","b")))
            .then((res)=>{
                expect(sendSpy).toBeCalledWith("fetch",{id: "___f___2",path: {"equals":"b"},sort:{}})
                expect(res).toEqual([4,3,2])
                done()
            })
        })
    ...
    describe ("Should test unfetching",()=>{
        ...
        it("Should unfetch full",(done)=>{
            const sendSpy= jest.fn().mockImplementation((method)=>(method==="info")?Promise.resolve({features:{fetch: "full"}}):Promise.resolve([]))

            jest.spyOn(JsonRPC,"default").mockReturnValue({...fullFetcherPeer(),send:sendSpy})
            const peer = new Peer()
        ...
        it("Should unfetch simple",(done)=>{
            const sendSpy= jest.fn().mockImplementation((method)=>(method==="info")?Promise.resolve({features:{fetch: "simple"}}):Promise.resolve([]))
            jest.spyOn(JsonRPC,"default").mockReturnValue({...simpleFecherPeer(),send:sendSpy})
            const peer = new Peer()
            const fetcher = new Fetcher()
