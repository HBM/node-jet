...
describe("Testing Logging",()=>{
    ...
    describe ("Should test callbacks",()=>{
        ...
        it("Schould create sock logger",()=>{
            ...
            const logger = new Logger({loglevel: LogLevel.socket,logname:"Foo",logCallbacks: [logSpy]})
            log(logger)
            expect(logSpy).toHaveBeenCalledTimes(11)
        })
        it("Schould create debug logger",()=>{
            const logSpy = jest.fn()
            const logger = new Logger({loglevel: LogLevel.debug,logname:"Foo",logCallbacks: [logSpy]})
            log(logger)
            expect(logSpy).toHaveBeenCalledTimes(9)
        })
        it("Schould create info logger",()=>{
            const logSpy = jest.fn()
            const logger = new Logger({loglevel: LogLevel.info,logname:"Foo",logCallbacks: [logSpy]})
            log(logger)
            expect(logSpy).toHaveBeenCalledTimes(6)
        })
        it("Schould create warn logger",()=>{
            const logSpy = jest.fn()
            const logger = new Logger({loglevel: LogLevel.warn,logname:"Foo",logCallbacks: [logSpy]})
            log(logger)
            expect(logSpy).toHaveBeenCalledTimes(4)
        })
        it("Schould create error logger",()=>{
            const logSpy = jest.fn()
            const logger = new Logger({loglevel: LogLevel.error,logname:"Foo",logCallbacks: [logSpy]})
            log(logger)
    ...
    describe ("Should test file",()=>{
        ...
        it("Schould create sock logger",(done)=>{
            const logger = new Logger({loglevel: LogLevel.socket,logname:"Foo",logfile:"tmp/sock.txt"})
            log(logger).then(()=>{
        ...
        it("Schould create debug logger",(done)=>{
            const logger = new Logger({loglevel: LogLevel.debug,logname:"Foo",logfile:"tmp/debug.txt"})
            log(logger).then(()=>{
        ...
        it("Schould create info logger",(done)=>{
            const logger = new Logger({loglevel: LogLevel.info,logname:"Foo",logfile:"tmp/info.txt"})
            log(logger).then(()=>{
        ...
        it("Schould create warn logger",(done)=>{
            const logger = new Logger({loglevel: LogLevel.warn,logname:"Foo",logfile:"tmp/warn.txt"})
            log(logger).then(()=>{
        ...
        it("Schould create error logger",(done)=>{
            const logger = new Logger({loglevel: LogLevel.error,logname:"Foo",logfile:"tmp/error.txt"})
            log(logger).then(()=>{
