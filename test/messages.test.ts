import {castMessage, ConfigureRequest, InfoRequest} from "../src/3_jet/messages"
...
describe("Testing message casting",()=>{
        ...
        it ("Configure error",()=>{ 
            try{
                const msg = castMessage<ConfigureRequest>({id:"abc",method:"configure",params:{foo:"bar"}} as any)
            }catch(ex){
                expect(ex.message).toEqual("Invalid params")
            }
        })
        it ("Should parse Configure",()=>{ 
            const msg = castMessage<InfoRequest>({id:"abc",method:"configure",params:{name:"Peer 1"}} as any)
            expect(typeof msg).toEqual("object")
        })
        it ("Add without path",()=>{
            try{
        ...
        it ("Should parse Add",()=>{
                const msg = castMessage<InfoRequest>({id:"abc",method:"add",params:{path:"myFunction"}} as any)
                expect(typeof msg).toEqual("object") 
        })
        ...
        it ("Should parse Remove",()=>{
                const msg = castMessage<InfoRequest>({id:"abc",method:"remove",params:{path:"myFunction"}} as any)
                expect(typeof msg).toEqual("object") 
        })
        it ("fetch without id",()=>{
            try{
                castMessage<InfoRequest>({id:"abc",method:"fetch",params:{path:{"equals":"foo"}}} as any)
            }catch(ex){
                expect(ex.message).toEqual("Invalid params")
            }
        })
        it ("Should parse fetch",()=>{
                const msg = castMessage<InfoRequest>({id:"abc",method:"fetch",params:{path:{"equals":"foo"},id:"__f__1"}} as any)
                expect(typeof msg).toEqual("object") 
        })
        ...
        it ("Should parse Unfetch",()=>{
                const msg = castMessage<InfoRequest>({id:"abc",method:"unfetch",params:{id:"__f__1"}} as any)
                expect(typeof msg).toEqual("object") 
        })
        it ("Change without value",()=>{
            try{
                castMessage<InfoRequest>({id:"abc",method:"change",params:{path:"foo"}} as any)
            }catch(ex){
                expect(ex.message).toEqual("Invalid params")
            }
        })
        it ("Should parse change",()=>{
                const msg = castMessage<InfoRequest>({id:"abc",method:"change",params:{path:"foo",value: 4}} as any)
                expect(typeof msg).toEqual("object") 
        })
